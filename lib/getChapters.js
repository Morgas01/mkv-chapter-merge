...
module.exports=function(files,{skipHidden=true}={})
{
	...
	.then(fileChapters=>
	{
		...
		for(let index=0;index<files.length;index++)
		{
			...
			if(fileTags.Chapters)
			{
				...
				.map((chapter,chapterIndex)=> // map chapter infos
				{
					let name;
					...
					let hidden=false;
					if(chapter.ChapterFlagHidden) hidden=chapter.ChapterFlagHidden.value!=0
					...
					return {name,uid,segmentUID,segmentEditionUID,timeStart,timeEnd,hidden,_raw:chapter};
				});
			}
			...
			//fill in chapter end timestamps
			...
			if(skipHidden) //filter hidden
			{
				chapters=chapters.filter(c=>!c.hidden);
			}
